...
export default async (req, res) => {
  ...
  if (!membershipStatus || !["active", "inactive"].includes(membershipStatus)) {
    return res.status(400).json({
      resultMessage:
        "Invalid or missing membershipStatus. Please provide a valid status ('active' or 'inactive').", // Invalid or missing membershipStatus
      resultCode: "00022",
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      resultMessage:
        "Successfully retrieved customers based on membership status.", // Success message
      resultCode: "00089",
      customers,
    });
  } catch (err) {
    ...
    return res.status(500).json({
      resultMessage:
        "Internal server error while fetching customers by membership status.", // Error message
      resultCode: "00008",
      error: err.message,
    });
  }
