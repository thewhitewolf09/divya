...
export default async (req, res) => {
  ...
  if (!startDate || !endDate) {
    return res.status(400).json({
      resultMessage: "Start date and end date are required.",
      resultCode: "00023",
    });
  }
  ...
  try {
    ...
    // Prepare report data
    const report =
      sales.length > 0 ? sales[0] : { totalSold: 0, totalRevenue: 0 };

    return res.status(200).json({
      resultMessage: "Sales report retrieved successfully.",
      resultCode: "00089",
      report,
    });
  } catch (err) {
    ...
    return res.status(500).json({
      resultMessage: "An error occurred while fetching the sales report.",
      resultCode: "00090",
      error: err.message,
    });
  }
