...
export default async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      return res.status(404).json({
        resultMessage: "Product not found", // Product not found error
        resultCode: "00093",
      });
    }
    ...
    if (existingAttendanceIndex !== -1) {
      ...
      if (dailyItem.attendance[existingAttendanceIndex].taken) {
        return res.status(400).json({
          resultMessage: "Attendance already marked for this date", // Attendance already taken error
          resultCode: "00094",
        });
      }
    ...
    return res.status(200).json({
      resultMessage: "Successfully recorded attendance and updated sale", // Success message
      resultCode: "00089",
      customer,
    });
  } catch (err) {
